...
const route=useRoute(); const router=useRouter(); const patients=ref<any[]>([]); const questionnaires=ref<any[]>([]); const result=ref<any>(); const qr=ref(''); const saving=ref(false)
const form=reactive({ patient_id: Number(route.query.patient)||undefined as number|undefined, questionnaire_version_ids: [] as number[], title:'认知筛查任务', note:'请根据真实感受完成问卷。', deadline:'' })
onMounted(async()=>{ const [p,q]=await Promise.all([api.get('/patients',{params:{page_size:100}}),api.get('/questionnaires')]); patients.value=p.data.items; questionnaires.value=q.data })
