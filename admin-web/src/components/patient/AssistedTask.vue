...
const trailGuideLines = computed(() => trailSequence.value.map(id => trailNodes.value.find(node => node.id === id)!).slice(1).map((node,index) => ({ from:trailNodes.value.find(item => item.id === trailSequence.value[index])!, to:node })))
      ...
      <template v-if="!sttStarted">
        ...
        <div class="stt-setup">
          ...
          <el-radio-group v-model="sttAgeBand">
            <el-radio-button v-for="band in sttAgeBands" :key="band.value" :label="band.value">{{ band.label }}</el-radio-button>
          </el-radio-group>
          <label>试卷</label>
          <el-radio-group v-model="sttForm">
            <el-radio-button label="A">A 卷（数字）</el-radio-button>
            <el-radio-button label="B">B 卷（数字 + 字母）</el-radio-button>
          </el-radio-group>
        </div>
      ...
      <template v-else>
        ...
        <div class="trail-board" aria-label="形状连线任务画布">
          <svg viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
            <line v-for="(line,index) in trailGuideLines" :key="`guide-${index}`" :x1="line.from.x" :y1="line.from.y" :x2="line.to.x" :y2="line.to.y" class="trail-guide-line"/>
            <line v-for="(line,index) in trailLines" :key="index" :x1="line.from.x" :y1="line.from.y" :x2="line.to.x" :y2="line.to.y"/>
          </svg>
          <button v-for="node in trailNodes" :key="node.id" :style="{left:`${node.x}%`,top:`${node.y}%`}" :class="[node.shape,{done:trailSequence.slice(0,trailProgress).includes(node.id),wrong:lastWrongNode===node.id,next:node.id===trailSequence[trailProgress]}]" :disabled="busy || trailSequence.slice(0,trailProgress).includes(node.id)" @click="clickTrail(node.id)">{{ node.label }}</button>
        </div>
