...
随后进一步收紧导入入口：内置目录和外部 JSON 均必须回传预览生成的 `content_hash` 及当时的基础版本。未预览、预览后内容变化或并发产生了新版本时统一返回 `409`，避免绕过管理页面直接写入未经核对的草稿。

对应提交：`e4fcf05 fix: require previews for all questionnaire imports`。

### 3.8 前端按需组件与稳定分包

路由原本已经使用动态导入，本次保留页面级懒加载，重点移除 Element Plus 全量插件注册：

- `main.ts` 只注册页面实际使用的组件、Loading 指令及其样式；
- `App.vue` 使用 `ElConfigProvider` 保留中文区域设置；
- Vite 将 Vue、Element Plus、ECharts 和 zrender 拆为稳定缓存包；
- ECharts 继续只随统计图页面加载，二维码依赖继续只随派发页面加载；
...
| 问卷治理 | 后端闭环与旧 SQLite 迁移测试 | 10 项通过。 |
| 前端性能 | TypeScript + Vite 生产构建 | 通过，无大 chunk 警告。 |
...
最初确认 Git 配置依赖本地 7890 代理，代理未启动时无法推送；移除该代理设置后，最近一次自动推送改为直接连接 GitHub，但当前受限执行环境仍无法连接 `github.com:443`。这不影响本地提交，但会导致 `dl` 领先 `origin/dl`。

处理建议：

1. 若使用代理，确认代理软件已启动且监听配置端口；若不使用代理，确认 Git 没有遗留无效代理设置；
...
- 本地 `dl` 已包含时间修复、问卷版本治理、全来源强制预览和前端分包优化；`7b931ca`、`ce1378f`、`e4fcf05` 及本次文档记录尚需在可访问 GitHub 的终端一并推送。
