...
Vue 医生端
  ...
  -> server/app/routers/*
  -> server/app/services/*
  -> server/app/models.py / SQLAlchemy
...
1. 在 `server/app/services/scale_catalog.py` 增加稳定 code、schema 和 scoring 配置。
2. 简单求和使用 `metadata_sum`；复杂算法在 `server/app/services/scoring.py` 增加明确策略。
3. 在 `server/tests/test_flow.py` 增加边界和期望分数。
4. 管理员先执行无写入预览，再导入草稿，核对版本差异并二次确认发布。
5. 医生派发返回的 `questionnaire_version_id`，患者端动态渲染，无需为每张简单量表新增页面。
6. 同步更新 README 和 `contracts/API.md`。
...
`server/app/modules/` 是 yjj 留下的分模块草案，存在 `due_at/deadline`、`patient_url/patient_link` 等字段差异。`server/app/main.py` 当前只注册 `server/app/routers/`，因此所有联调和文档均以后者为准。
...
逐项操作、验证结果、服务故障和恢复方法见 [来源开发记录](DEVELOPMENT_LOG.md)。
