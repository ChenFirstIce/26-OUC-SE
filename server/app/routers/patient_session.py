...
def tasks(identity: PatientIdentity = Depends(patient_identity), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "items": [{
            ...
            "submitted_at": item.response.submitted_at if item.response else None,
            "duration_seconds": item.response.duration_seconds if item.response else None,
...
def submit(
    ...
    if not response:
        response = Response(assignment_item_id=item.id, answers_json=payload.answers, revision=1)
        db.add(response)
        db.flush()  # Materialize started_at before calculating direct-submission duration.
    response.answers_json = payload.answers
